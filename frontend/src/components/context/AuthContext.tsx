...
import {UserApi, UserProfile, UserProfileResponse} from "@/src/codegen";

interface AuthContextProps {
  authUser?: User,
  siteUser?: UserProfile
}
...
export const AuthContextProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const [siteUser, setSiteUser] = React.useState<UserProfile | undefined>(undefined);
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        new UserApi(headerConfig(token)).getUserprofile().then(response => {
          ...
          setSiteUser(profileResponse.profile)
        }).catch(e => ERROR(e))
  ...
  return (
      <AuthContext.Provider value={{ authUser, siteUser }}>
