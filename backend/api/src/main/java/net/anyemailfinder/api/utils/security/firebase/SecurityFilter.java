package net.anyemailfinder.api.utils.security.firebase;
...
import net.anyemailfinder.api.utils.firebase.FirebaseService;
import net.anyemailfinder.api.utils.security.Credentials;
import net.anyemailfinder.api.utils.security.UnsecurePaths;
...
public class SecurityFilter extends OncePerRequestFilter {
  ...
  private void setEntitlements(SessionUser user) {
    Optional<UserEntity> userEntityOpt = userRepo.findById(user.getUserId());
    if (userEntityOpt.isPresent()) {
      user.setPremiumUser(userEntityOpt.get().getIsPremiumUser());
      if (user.isPremiumUser()) {
        user.setMaxLeadLimit(Long.MAX_VALUE);
      } else {
        user.setMaxLeadLimit(Math.max(0,
            entitlementProps.getMaxLeadCountFree() -
                userLeadsRepo.leadCountByUserId(user.getUserId())));
      }
    } else {
      user.setPremiumUser(false);
      user.setMaxLeadLimit(entitlementProps.getMaxLeadCountFree());
    }
  }
